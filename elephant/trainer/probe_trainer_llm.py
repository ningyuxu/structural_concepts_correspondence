...
class LLMProbeTrainer(object):
    ...
    def train(
        ...
        s_dataset_cfg = {
            "lang": s_lang,
            "genre": self.corpus.corpus_cfg.corpora[f"{s_lang}_train"]["genre"],  # elephant.config.source_langs.criteria.genres[s_lang],
            "predict_mode": self.trainer_cfg.predict_mode
        }
        self.dataset = Dataset(self.producer, **s_dataset_cfg)
        self.checkpoint_path = elephant.checkpoint_path / self.trainer_cfg.objective / f"{s_lang}"
        self.checkpoint_path.mkdir(exist_ok=True, parents=True)

        self.log_path = elephant.log_path / self.trainer_cfg.objective / f"{s_lang}"
        ...
        src_checkpoint_path = elephant.checkpoint_path / self.trainer_cfg.objective / f"{s_lang}"
        ...
        # self.model.load_state_dict(self.model.load(src_checkpoint_path / "best_model.pt").state_dict(), strict=False)
        # self.model.align_module.switch_mapping(mapping_type="residual")
        ...
        try:
            ...
            for epoch in range(epoch + 1, self.max_epochs + 1):
                ...
                for batch_no in range(0, num_iterator):
                    ...
                    loss_result, count = self.model.forward_loss(batch=batch, protos_dict=self.s_protos_dict)
                ...
                if log_dev:
                    ...
                    dev_metric_result, dev_loss_result = self.model.evaluate(
