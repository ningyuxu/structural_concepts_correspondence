...
class MetaProtoNetTrainer(object):
    ...
    def train(
        ...
        # ----------------------------------------------------------------------------------------------------
        # elephant.checkpoint_path / self.trainer_cfg.objective / f"{s_lang}_probe"
        ...
        try:
            ...
            logger.info(f'- adaptor_beta: "{elephant.config.model.task.proto_module.adaptor_beta}"')
            ...
            for epoch in range(epoch + 1, self.max_epochs + 1):
                ...
                for task_no in range(0, num_iterator):
                    ...
                    loss_result, count = self.model.forward_loss(
                        ...
                        s_protos_dict=self.s_protos_dict,
                    ...
                    optimizer.step()
                    self.model.orthogonalize_uni_proj()
                    average_over += count
        ...
        if self.t_dataset.test:
            log_dir = elephant.log_path / self.trainer_cfg.objective / \
                      f"results-{self.trainer_cfg.task}-protonet-{k_shots}" / s_lang
    ...
    def final_test(
        ...
        # get support set from training dataset
    ...
    @staticmethod
    def select_valid_labels(s_lang, t_lang, task, threshold=30):
    ...
    def get_valid_labels_lang_pair(self, s_lang: str, langs: List = None):
