...
class TokenDropout(nn.Module):
    """
    Seek to randomly zero the vectors of some tokens with the probability of `p`.
...
class SharedDropout(nn.Module):
    """
    Differs from the vanilla dropout strategy in that the dropout mask is shared across one dimension.
...
class IndependentDropout(nn.Module):
    ...
    When :math:`N-M` of them are dropped, the remaining :math:`M` ones are scaled by a factor of :math:`N/M` to
    compensate, and when all of them are dropped together, zeros are returned.
    ...
    def forward(self, *items: torch.Tensor) -> List[torch.Tensor]:
        if not self.training:
            return list(items)
