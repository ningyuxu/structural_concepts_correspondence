...
class MetaProtoModel(ModelTemplate):
    def __init__(self, model_cfg):
        super(MetaProtoModel, self).__init__(model_cfg)
    ...
    def orthogonalize_uni_proj(self):
        self.proto_module.orthogonalize_uni_proj()
    ...
    def forward_loss(
        ...
        loss, count, detail_loss = self.proto_module.forward_loss(samples=samples)
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, leave=False):
                ...
                metric, loss = self.proto_module.evaluate(samples=batch)
